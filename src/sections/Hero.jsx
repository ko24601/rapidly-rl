...
export default function Hero() {
  ...
  return (
    <div style={{ position: 'relative', height: '100vh', minHeight: '640px', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>

      {/* BG */}
      <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${BASE}hero-bg.png)`, backgroundSize: 'cover', backgroundPosition: 'center' }} />
      <Particles />

      {/* Overlays */}
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(105deg, rgba(5,5,5,0.97) 0%, rgba(5,5,5,0.82) 55%, rgba(5,5,5,0.2) 100%)', zIndex: 2 }} />

      {/* Green glow blob */}
      <div style={{
        position: 'absolute', top: '38%', left: '-80px',
        width: '650px', height: '650px', borderRadius: '50%',
        background: 'radial-gradient(circle, rgba(57,255,20,0.14) 0%, transparent 68%)',
        pointerEvents: 'none', zIndex: 2,
        animation: 'heroPulse 5s ease-in-out infinite',
      }} />

      {/* Diagonal speed stripes right side */}
      <div style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: '42%', overflow: 'hidden', zIndex: 2, pointerEvents: 'none' }}>
        {Array.from({ length: 7 }).map((_, i) => (
          <div key={i} style={{
            position: 'absolute', top: '-20%', right: `${i * 80 - 40}px`,
      ...
      {/* Left accent bars */}
      <div style={{ position: 'absolute', left: 0, top: '50%', transform: 'translateY(-50%)', zIndex: 3, pointerEvents: 'none' }}>
        {[80, 52, 30].map((w, i) => (
          <div key={i} style={{
            height: '2px', width: `${w}px`,
            background: `rgba(57,255,20,${0.9 - i * 0.25})`,
            marginBottom: '7px',
      ...
      <div style={{ position: 'relative', zIndex: 4, maxWidth: '1200px', margin: '0 auto', padding: '0 40px', width: '100%' }}>
        ...
        <div style={{ position: 'relative', marginBottom: '28px' }}>
          <span aria-hidden="true" style={{
            ...
            fontSize: 'clamp(56px, 11vw, 128px)', letterSpacing: '-2px', textTransform: 'uppercase', lineHeight: 0.88,
            color: 'var(--primary)', opacity: 0.65, animation: 'glitch1 8s infinite', pointerEvents: 'none',
          }}>{name}</span>
          <span aria-hidden="true" style={{
            position: 'absolute', inset: 0,
            fontFamily: 'var(--font-heading)', fontWeight: 900,
            fontSize: 'clamp(56px, 11vw, 128px)', letterSpacing: '-2px', textTransform: 'uppercase', lineHeight: 0.88,
            color: '#ff3333', opacity: 0.4, animation: 'glitch2 8s infinite 0.4s', pointerEvents: 'none',
          }}>{name}</span>
          <motion.h1
            ...
            style={{
              ...
              fontSize: 'clamp(56px, 11vw, 128px)', letterSpacing: '-2px', textTransform: 'uppercase', lineHeight: 0.88,
              color: 'var(--text)',
              textShadow: '0 0 60px rgba(57,255,20,0.08)',
              animation: 'neonFlicker 12s infinite 4s',
            }}
          >{name}</motion.h1>
        </div>
        ...
        >
          <a href={SITE.discord} target="_blank" rel="noopener noreferrer"
            style={{ fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: '13px', letterSpacing: '2px', textTransform: 'uppercase', padding: '15px 36px', background: 'var(--primary)', color: '#050505', border: '2px solid var(--primary)', textDecoration: 'none', display: 'inline-block', boxShadow: '0 0 32px rgba(57,255,20,0.45), 0 0 64px rgba(57,255,20,0.15)', transition: 'all 0.25s' }}>
            Join Discord
          </a>
          <button onClick={() => scrollTo('calendar')}
            style={{ fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: '13px', letterSpacing: '2px', textTransform: 'uppercase', padding: '15px 36px', background: 'transparent', color: 'var(--text)', border: '2px solid rgba(255,255,255,0.18)', cursor: 'pointer', transition: 'all 0.25s' }}>
            View Schedule →
          </button>
        </motion.div>
      </div>
      ...
      <motion.div className="hero-widget"
        initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.6, duration: 0.6 }}
        style={{
          position: 'absolute', bottom: '48px', right: '40px',
          background: 'rgba(6,6,6,0.92)', backdropFilter: 'blur(16px)',
          border: '1px solid rgba(57,255,20,0.2)', borderLeft: '3px solid var(--primary)',
          padding: '22px 26px', fontFamily: 'var(--font-mono)', zIndex: 4,
      ...
      <style>{`
        @media(max-width:768px){.hero-widget{display:none!important}}
      `}</style>
    </div>
  )
}
